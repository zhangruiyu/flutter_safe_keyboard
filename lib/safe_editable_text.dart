...
///  * [SafeEditableText], which is a raw region of editable text that can be
...
class SafeEditableText extends StatefulWidget {
  ...
  @override
  SafeEditableTextState createState() => new SafeEditableTextState();
...
/// State for a [SafeEditableText].
class SafeEditableTextState extends State<SafeEditableText> with AutomaticKeepAliveClientMixin implements TextInputClient, TextSelectionDelegate {
  Timer _cursorTimer;
  final ValueNotifier<bool> _showCursor = new ValueNotifier<bool>(false);
  final GlobalKey _editableKey = new GlobalKey();

  SafeTextInputConnection _textInputConnection;
  ...
  @override
  void didUpdateWidget(SafeEditableText oldWidget) {
  ...
  void _openInputConnection() {
    if (!_hasInputConnection) {
      final TextEditingValue localValue = _value;
      _lastKnownRemoteTextEditingValue = localValue;
      _textInputConnection = SafeTextInput.attach(this,
  ...
  void _closeInputConnectionIfNeeded() {
    if (_hasInputConnection) {
      _textInputConnection.close(context);
